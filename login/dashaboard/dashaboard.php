...
$sql = "SELECT COUNT(*) as count FROM signs WHERE x = 0 AND y = 0
  AND sender_id = (SELECT id FROM users WHERE username = '$username')";
$result = $conn->query($sql);
if ($result && $result->num_rows > 0) { 
  $row = $result->fetch_assoc();
...
$sql = "SELECT COUNT(*) as count FROM signs WHERE is_sign='yes'
  AND sender_id = (SELECT id FROM users WHERE username = '$username')";
$result = $conn->query($sql);
if ($result && $result->num_rows > 0) { 
  $row = $result->fetch_assoc();
 $signed_docs = $row['count'];
}
	...
	<header class="menu-wrap">
		...
		<nav>
			<section class="dicover">
				...
				<ul>
					<li>
						<a href="pages\positions\positions.php">
					...
					<li>
						<a href="pages\refused\refuesd.php">
					...
					<li>
						<a href="pages/upload/upload.php">
			...
			<section class="info-boxes">
				<div class="info-box">
					...
					<div class="box-content">
						<span class="big"><?php echo  $signed_docs;?></span>
						Signed Documentes
					</div>
				</div>
				...
				<div class="info-box">
					...
					<div class="box-content">
						<span class="big"><?php echo  $positions;?></span>
					Positions
					</div>
				</div>
