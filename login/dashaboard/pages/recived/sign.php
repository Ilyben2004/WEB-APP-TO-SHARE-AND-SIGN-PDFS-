...
if ($result && $result->num_rows > 0) {
  ...
  $row = $result->fetch_assoc();
   ...
   <div class="pdfname">PDF File Name</div>
    <div class="recipe">The signer </div>
    <div></div>
    <?php
      $servername = "localhost";
      $user = "root";
	...
	$sql = "SELECT pdf_file, id_sign, recipe_id 
    FROM signs
    WHERE sender_id = (SELECT id FROM users WHERE username = '$username') and x<>0 and y <> 0 and is_sign='yes'";
...
while ($row = $result->fetch_assoc()) {
    ...
    // Check if the 'id_sign' key exists in the fetched row
  
        $idSign = $row['id_sign'];
        $sender_id= $row['recipe_id'];
        // Retrieve the PDF files associated with the sender ID (based on the username)
$sql = "SELECT nom,prenom
    ...
    // Display the PDF file name and ID Sign
    echo '<div class="pdfname">' . $pdfFile . '</div>';
	
    echo '<div class="recipe">' . $nom . ' ' . $prenom . '</div>';
  
    echo '<div class="linkto"><a href="download\download.php?var=' . urlencode(    $idSign) . '">VIEW THE PDF</a></div>';
