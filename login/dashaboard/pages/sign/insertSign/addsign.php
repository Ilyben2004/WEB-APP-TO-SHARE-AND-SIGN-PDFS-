...
if ($result && $result->num_rows > 0) {
    ...
    // Construct the image URL based on the image name
    $imageURL = '../../images/' . $img;
...
if (isset($_POST['submit'])) {
    ...
    if ($result && $result->num_rows > 0) {
       ...
       $pdf = new Fpdi();
// set the source file
$pdf->setSourceFile("../../positions/pdfs/". $pdfFile);

// Get the total number of pages in the original PDF
$pageCount = $pdf->setSourceFile("../../positions/pdfs/" .  $pdfFile);

// Loop through all pages in the original PDF
for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
    // Add a page to the modified PDF
    $pdf->AddPage();

    // Import the current page from the original PDF
    $tplId = $pdf->importPage($pageNo);

    // Get the size of the imported page
    $size = $pdf->getTemplateSize($tplId);

    // Calculate the scale factor to fit the imported page into the fixed output size
    $scaleFactor = min($pdf->GetPageWidth() / $size['width'], $pdf->GetPageHeight() / $size['height']);

    // Calculate the position to center the imported page
    $centerX = ($pdf->GetPageWidth() - $size['width'] * $scaleFactor) / 2;
    $centerY = ($pdf->GetPageHeight() - $size['height'] * $scaleFactor) / 2;

    // Use the fixed output size and position to display the imported page
    $pdf->useTemplate($tplId, $centerX, $centerY, $size['width'] * $scaleFactor, $size['height'] * $scaleFactor);

    // Check if this is the last page and add the image
    if ($pageNo === $pageCount) {
        $pdf->Image($imageURL,($x/5)-10 ,($y+130)/5, 50, 25);

    }
}

// Save the modified PDF to a new file
$outputFilePath = '../../positions/pdfs/'. $pdfFile;
